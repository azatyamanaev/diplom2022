...
public class RestClient {
    ...
    public RestClient(@Qualifier("apiRestTemplate") RestTemplate restTemplate,
                      @Qualifier("apiObjectMapper") ObjectMapper mapper) {
    ...
    public <T> RequestBuilder<T> post(String url, TypeReference<T> responseType) {
        return new RequestBuilder<>(HttpMethod.POST, url, responseType);
    }
    ...
    public <T> RequestBuilder<T> get(String url, TypeReference<T> responseType) {
        return new RequestBuilder<>(HttpMethod.GET, url, responseType);
    }
    ...
    public <T> RequestBuilder<T> put(String url, TypeReference<T> responseType) {
        return new RequestBuilder<>(HttpMethod.PUT, url, responseType);
    }
    ...
    public <T> RequestBuilder<T> patch(String url, TypeReference<T> responseType) {
        return new RequestBuilder<>(HttpMethod.PATCH, url, responseType);
    }
    ...
    public <T> RequestBuilder<T> delete(String url, TypeReference<T> responseType) {
        return new RequestBuilder<>(HttpMethod.DELETE, url, responseType);
    }
    ...
    public class RequestBuilder<T> {
        ...
        public String getString() {
            try {
                ...
                log.error("Can't map object to json. Cause: ", e);
            }
        ...
        public Optional<T> send() {
            try {
                ...
                if (resp.getStatusCode().isError()) {
                    log.error("Server response code - {}, response body - {}", resp.getStatusCodeValue(), resp.getBody());
                    if (errorHandler != null) {
                        responseBody.ifPresent(body -> errorHandler.accept(body, resp.getStatusCode()));
                    }
                } else if (log.isTraceEnabled()) {
                    log.trace("Response code - {}, response body - {}", resp.getStatusCodeValue(), resp.getBody());
                }
                return responseBody;
                ...
                if (cause instanceof ConnectTimeoutException || cause instanceof SocketTimeoutException) {
                    log.error("API timeout: {} ({} {})", cause, method, url);
                    throw new IllegalStateException();
                } else if (rce instanceof HttpClientErrorException) {
                    final HttpClientErrorException hcee = (HttpClientErrorException) rce;
                    log.error("Response code - {}, response body - {}", hcee.getRawStatusCode(), hcee.getResponseBodyAsString());
                }
                log.error("{} {} ({})", method, url, rce.getMessage(), rce);
            } catch (JsonProcessingException e) {
                log.error("Can't map object to json. Cause: ", e);
            }
        ...
        private T parseBody(final String body) {
            try {
                ...
                log.error(
                        "Can't parse body for method {} , url - \"{}\", body - {}",
                        method,
                        url,
                        body
                );
